...
from transformers import AutoModelForSeq2SeqLM, PretrainedConfig, T5Model


@dataclass
class ModelLoadingInfoT5:
    name: str
    tokenizer_name: str
    classification: bool
    ...
    def __post_init__(self):
        if self.model_class is None:
            self.model_class = T5Model if self.classification else AutoModelForSeq2SeqLM


def load_model(model_loading_info: ModelLoadingInfoT5):
...
def average_models(model_loading_infos: List[ModelLoadingInfoT5], config=None):
    """
...
if __name__ == '__main__':
    #todo change for T5
    bert = ModelLoadingInfoT5(name="bert-base-uncased", tokenizer_name="bert-base-uncased", classification=True)
    ...
    roberta = ModelLoadingInfoT5(name="roberta-base", tokenizer_name="roberta-base", classification=True)
    models_to_fuse = ['cross-encoder/stsb-roberta-base', 'textattack/roberta-base-STS-B']
    models_to_fuse = [ModelLoadingInfoT5(name=model, tokenizer_name=model, classification=True) for model in models_to_fuse]
